...
import type { Todo, FiltersKey, Filters } from '@/types';
...
const todos: Ref<Array<Todo>> = ref(JSON.parse(sessionStorage.getItem('todos') || '[]'));
...
const filteredTodos = computed<Todo[]>(() => filter[visibility.value](todos.value))
...
<template>
  <section class="todoapp">
    ...
    <section class="main">
      <input id="toggle-all" class="toggle-all" type="checkbox" :checked="(remaining===0)" @change="toggle">
      <label for="toggle-all">Mark all as complete</label>
      <ul class="todo-list">
        <li class="todo" :class="{completed: todo.completed, editing: todo === editedTodo}" v-for="todo of filteredTodos">
          <div class="view">
